...
public class RenderImage
{
	...
	public void Dispose()
	{
		...
		DestroyTexture();
		if(this._image.gOwner != null){
			GGraph g = (GGraph)this._image.gOwner;
			g.SetNativeObject(null);
			this._image.onRemovedFromStage.Remove(OnRemoveFromStage);
			OnRemoveFromStage();
		}
		this._image.Dispose();
		this._image = null;
	...
	{
		if(_renderTexture == null){
			Debug.Log("Error _renderTexture is null");
			return;
		}
	...
	public void SetCamLockModel(){
		if (_model == null || _camera == null){
			return;
		}
		SkinnedMeshRenderer mesh = _model.GetComponentInChildren<SkinnedMeshRenderer>();
		if(mesh == null){
			return;
		}
		modelRoot.localPosition = new Vector3(0, 0, 1);
		modelRoot.localScale = new Vector3(1, 1, 1);
        modelRoot.localRotation = Quaternion.Euler(0, 120, 0);
		// mesh.bounds.center
		var bounds = mesh.bounds;
		// Debug.Log("bounds: "+ bounds.ToString());
		var b = _camera.transform.position;
		var pos = bounds.center - b;
		// Debug.Log("pos: "+ pos.ToString());
		// Debug.Log("extents: " + bounds.extents.ToString());
		float s = bounds.extents.x;
		if(s < bounds.extents.y)
			s = bounds.extents.y;
		if(s < bounds.extents.z)
			s = bounds.extents.z;
		float y = -pos.y;
		// Debug.Log("max: "+ s.ToString());
		if(s < 0.5f){
			modelRoot.localPosition = new Vector3(0, y, 1.1f + s);
		}else
		{
			modelRoot.localPosition = new Vector3(0, y, 1.1f + s * 3.2f);	
		}
	...
	public void SetShader( string shaderName)
	{
		...
		// mesh.materials = mats;
        // if (_renderImage.modelRoot.childCount > 0)
        // {
        //     var model = _renderImage.modelRoot.GetChild(0);
        //     var renders = model.GetComponentsInChildren<SkinnedMeshRenderer>();
        //     foreach (SkinnedMeshRenderer sourceRenderer in renders)
        //     {
        //         Material[] mats = sourceRenderer.sharedMaterials;
        //         for (int i = 0; i < mats.Length; i++)
        //         {
        //             mats[i].shader = shader;
        //         }
        //         sourceRenderer.materials = mats;
        //     }
        // }
	}
}