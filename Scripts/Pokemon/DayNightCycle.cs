...
public class DayNightCycle : MonoBehaviour
{
    ...
    public Light lightChain;

    public Color filterChain = new Color(1, 1, 1, 1);
    ...
    */
    ...
    public void SetCustomTime(int customTime){
        if(customTime >= 24){
            customTime = customTime % 24;
        }
        if(customTime < 0){
            customTime = 24 - (-customTime % 24);
        }
        hour = Mathf.FloorToInt(customTime);
        minute = Mathf.FloorToInt((customTime - hour) * 60);
    }
    ...
    public void UpdateTime(){
        ...
        if(lightChain != null){
            ...
            lightChain.color = Color.Lerp( Color.white, c, 0.5f);
        }
    }
}