...
namespace AON.RpgMapEditor
{
    ...
    public class RpgMapHelper
    {
        ...
        {
            return AutoTileMap_Editor.Instance.ViewCamera.ScreenToWorldPoint(new Vector3(Input.mousePosition.x, Input.mousePosition.y));
        }
        ...
        {
            vPos -= AutoTileMap_Editor.Instance.transform.position;

            // transform to pixel coords
            vPos.y = -vPos.y;
            vPos.x = vPos.x * AutoTileMap_Editor.Instance.Tileset.TileWidth / AutoTileMap_Editor.Instance.CellSize.x;
            vPos.y = vPos.y * AutoTileMap_Editor.Instance.Tileset.TileHeight / AutoTileMap_Editor.Instance.CellSize.y;

            if (vPos.x >= 0 && vPos.y >= 0)
            {
                int tile_x = (int)vPos.x / AutoTileMap_Editor.Instance.Tileset.TileWidth;
                int tile_y = (int)vPos.y / AutoTileMap_Editor.Instance.Tileset.TileHeight;
                return tile_y * AutoTileMap_Editor.Instance.MapTileWidth + tile_x;
            }
            else
            {
                return -1;
        ...
        public static AutoTile GetAutoTileByPosition( Vector3 vPos, int iLayer )
        {
            ...
            int tileX = (int)vPos.x / AutoTileMap_Editor.Instance.Tileset.TileWidth;
            int tileY = (int)vPos.y / AutoTileMap_Editor.Instance.Tileset.TileHeight;
            if (vPos.x < 0) tileX -= 1;
            if (vPos.y < 0) tileY -= 1;
            return AutoTileMap_Editor.Instance.GetAutoTile(tileX, tileY, iLayer);
        }
        ...
        public static void SetAutoTileByPosition( Vector3 vPos, int tileId, int iLayer )
        {
            ...
            if (vPos.x >= 0 && vPos.y >= 0)
            {
                int tile_x = (int)vPos.x / AutoTileMap_Editor.Instance.Tileset.TileWidth;
                int tile_y = (int)vPos.y / AutoTileMap_Editor.Instance.Tileset.TileHeight;
                AutoTileMap_Editor.Instance.SetAutoTile( tile_x, tile_y, tileId, iLayer, true);
            }
        }
        ...
        {
            Vector3 vPos = new Vector3((tile_x + 0.5f) * AutoTileMap_Editor.Instance.CellSize.x, -(tile_y + 0.5f) * AutoTileMap_Editor.Instance.CellSize.y);
            return vPos;
        }
        ...
        public static float Raycast( Ray2D ray, float distance, float precission = 0f )
        {
            if( precission <= 0 )
            {
                precission = AutoTileMap_Editor.Instance.CellSize.x / 2f;
            }
            Vector2 vInc = ray.direction.normalized * precission;
            Vector2 curPos = ray.origin;
            for (float d = 0; d <= distance; d += precission )
            {
                // eTileCollisionType collType = AutoTileMap_Editor.Instance.GetAutotileCollisionAtPosition(curPos);
        ...
        public static void RemoveFogOfWar(Vector3 worldPosition, int sightLength)
        {
            ...
            int iFogLayer = AutoTileMap_Editor.Instance.MapLayers.FindIndex(x => x.LayerType == eLayerType.FogOfWar);
            if (iFogLayer >= 0)
            {
                ...
                for (int yf = -sightLength; yf <= sightLength; ++yf)
                {
                    for (int xf = -sightLength; xf <= sightLength; ++xf)
                    {
                        ...
                        if (AutoTileMap_Editor.Instance.IsValidAutoTilePos(tx, ty)) // This extra check is needed with fog of war layer
                        {
                            AutoTile autoTile = AutoTileMap_Editor.Instance.GetAutoTile(tx, ty, iFogLayer);
                ...
                AutoTileMap_Editor.Instance.RefreshMinimapTexture(centerTile.TileX - sightLength, centerTile.TileY - sightLength, 2 * sightLength, 2 * sightLength);
            }
            */
        }
                ...
                for (int yf = -sightLength; yf <= sightLength; ++yf)
                {
                    for (int xf = -sightLength; xf <= sightLength; ++xf)
                    {
                        ...
                        if (AutoTileMap_Editor.Instance.IsValidAutoTilePos(tx, ty)) // This extra check is needed with fog of war layer
                        {
                            ...
                            // AutoTileMap_Editor.Instance.AddFogOfWarSetToQueue(tx + ty * AutoTileMap_Editor.Instance.MapTileWidth, aFogAlpha);
                        }
                    }
                }
